...
from .forms import StudentForm  # Assuming you have a form for RegisterStudent

def register_student(request):
    student_form = StudentForm(request.POST or None, request.FILES or None)
    context = {'form': student_form, 'page_title': 'Register Student'}
    
    if request.method == 'POST':
        if student_form.is_valid():
            first_name = student_form.cleaned_data.get('firstname')
            last_name = student_form.cleaned_data.get('lastname')
            address = student_form.cleaned_data.get('address')
            email = student_form.cleaned_data.get('email')
            gender = student_form.cleaned_data.get('gender')
            password = student_form.cleaned_data.get('password')
            father_name = student_form.cleaned_data.get('fathername')
            father_occupation = student_form.cleaned_data.get('fatheroccupation')
            mother_name = student_form.cleaned_data.get('mothername')
            mother_occupation = student_form.cleaned_data.get('motheroccupation')
            ...
            try:
                user = RegisterStudent.objects.create(
                    ...
                    DOB=dob, mark_10=mark_10, mark_12=mark_12
                )
                messages.success(request, "Successfully Added")
...
def view_student(request):
    data = RegisterStudent.objects.all()
    context = {
        'data': data,
